...
void ActiveManager::LoadActiveConfg()
{
	...
	if (tabFile.Load(filename))
	{
		...
		while (lineItem.size() >= 65 )
		{
			...
			active->mActiveName = lineItem[1].c_str();
			active->mActiveIntroduction = lineItem[2].c_str();
			active->mBeginTime = lineItem[3].c_str();
			active->mEndTime = lineItem[4].c_str();
			active->mRewardNum = atoi(lineItem[5].c_str());
			active->mAllPercent = 0;
			for (Lint i = 0; i < MAX_GIFT_NUM; i ++)
			{
				if (lineItem[i * 4 + 5] != "" && lineItem[i * 4 + 6] != "" && lineItem[i * 4 + 7] != "")
				{
					active->mGift[i].giftname = lineItem[i * 5 + 6].c_str();
