#ifndef    _DDZ_HANDLER_SANLAI_H_
#define    _DDZ_HANDLER_SANLAI_H_
...
#define USER_COUNT_3LAI (3)
class GameHandler_DDZ_3lai : public GameHandler
{
public:
...
private:
	...
	CardVec         m_handCard[USER_COUNT_3LAI];//手上的牌 ;
	CardVec         m_leftCard;//斗地主底牌;

	CardVec			m_outCard[USER_COUNT_3LAI];

	Lchar           m_playerCardType[USER_COUNT_3LAI];//出牌类型;
	Lchar           m_curCardType;
	Lchar           m_curCardMode;
	...
	Lint            m_bJiaoOrNo[USER_COUNT_3LAI]; //首先玩家是否叫了地主，以此判断该玩家是否可以抢地主;

	Lint			m_bomb[USER_COUNT_3LAI];
	Lint            m_nUserBeishu;

	CardDeal_3Lai		m_dealCard;
	...
	CardHelp_3Lai		m_help;
	std::vector<std::vector<Lint>>			m_curHelpInfo;
	std::vector<Lint>                       m_curHelpUnusable;
	Lint			m_baojing[USER_COUNT_3LAI];
};

DECLARE_GAME_HANDLER_CREATOR(game_ddz_3lai, GameHandler_DDZ_3lai)
