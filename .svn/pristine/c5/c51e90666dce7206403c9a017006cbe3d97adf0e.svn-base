...
bool sort_fun(Card_Wakeng* p1, Card_Wakeng* p2)
{
	if (p1->GetNumberType() != p2->GetNumberType())
	{
		return p1->GetNumberType() > p2->GetNumberType();
	}
	else
	{
...
bool sort_fun_by_laizi(Card_Wakeng* p1, Card_Wakeng* p2)
{
	BYTE b1, b2;
	if (p1->GetLaizi())
	{
...
//////////////////////////////////////////////////////////////////////////
CardDeal_Shanxi_Wakeng::CardDeal_Shanxi_Wakeng()
{
	m_pCard.resize(CARD_COUNT_WAKENG);
	for(int i = 0 ; i < CARD_COUNT_WAKENG; ++i)
	{
...
void CardDeal_Shanxi_Wakeng::SS(Lint nCardPos, Lint nColor, Lint nNumber)
{
...
void CardDeal_Shanxi_Wakeng::TiaoPai(CardValue tiaopai[], Lint nLenth)
{
...
void CardDeal_Shanxi_Wakeng::DealCard(CardVec_Wakeng& vec1, CardVec_Wakeng& vec2, CardVec_Wakeng& vec3, Lint cardCount, CardValue* tiaopai, Lint nLenth)
{
	RandCard();
	assert(cardCount > 0);
	...
	int n = 0;
	vec1.insert(vec1.end(), m_pCard.begin() + n, m_pCard.begin() + n + cardCount);
	n += cardCount;
	vec2.insert(vec2.end(), m_pCard.begin() + n, m_pCard.begin() + n + cardCount);
	n += cardCount;
	vec3.insert(vec3.end(), m_pCard.begin() + n, m_pCard.begin() + n + cardCount);
}

void CardDeal_Shanxi_Wakeng::GetLeftCard(CardVec_Wakeng& leftCard)
{
	leftCard.insert(leftCard.end(), m_pCard.begin() + 16*3, m_pCard.end());
}

void CardDeal_Shanxi_Wakeng::SetLaizi(BYTE bCardNumber)
{
	for(int i = 0 ; i < CARD_COUNT_WAKENG; ++i)
	{
...
void CardDeal_Shanxi_Wakeng::ClearLaizi()
{
	for(int i = 0 ; i < CARD_COUNT_WAKENG; ++i)
	{
...
void CardDeal_Shanxi_Wakeng::RandCard()
{
	m_pCard.clear();
	for(int i = 0 ; i < CARD_COUNT_WAKENG; ++i)
	{
	...
	for(int i = 0 ; i < seed; ++i)
	{
		...
		Card_Wakeng* tmp = m_pCard[seed1];
		m_pCard[seed1] = m_pCard[seed2];
		m_pCard[seed2] = tmp;
	}
	seed = L_Rand(0,m_pCard.size()-1);
	CardVec_Wakeng tmp(m_pCard.begin(),m_pCard.begin()+seed);
	tmp.insert(tmp.end(),m_pCard.begin()+seed,m_pCard.end());
	m_pCard = tmp;
}

//////////////////////////////////////////////////////////////////////////
CardStyle_Shanxi_Wakeng::CardStyle_Shanxi_Wakeng()
{
}
...
void CardStyle_Shanxi_Wakeng::SortCard(CardVec_Wakeng& vec)
{
	std::sort(vec.begin(),vec.end(),sort_fun);
}

void CardStyle_Shanxi_Wakeng::SortCardLaiziFront(CardVec_Wakeng& vec)
{
	std::sort(vec.begin(),vec.end(),sort_fun);

	CardVec_Wakeng Normal,Laizi;
...
void CardStyle_Shanxi_Wakeng::SortCardLaiziInsert(CardVec_Wakeng& vec)
{
	std::sort(vec.begin(),vec.end(),sort_fun_by_laizi);
}

void CardStyle_Shanxi_Wakeng::EarseCard(CardVec_Wakeng& des,CardVec_Wakeng& src)
{
	for(CardVec_Wakeng::iterator sit = src.begin();sit != src.end();++sit)
	{
		for(CardVec_Wakeng::iterator dit = des.begin();dit != des.end();)
		{
			if(*sit == *dit)
			{
				dit = des.erase(dit);
...
int CardStyle_Shanxi_Wakeng::CanBig(DWORD dsCount,BYTE bsStyle ,BYTE bsNumber,CardStyleNumberVec& style, bool isUseBomb)
{
...
bool CardStyle_Shanxi_Wakeng::CheckHasBomb(CardVec_Wakeng const& Src, Lint* pBombCard)
{
	CardVec_Wakeng one,tow,three,four;
	CardStyle_Shanxi_Wakeng::Instance()->GetSingleDoubleThreeFour(Src,one,tow,three,four);
	if (four.size() > 0)
	{
		if (pBombCard != NULL)
		{
...
void CardStyle_Shanxi_Wakeng::CheckCardStyle(CardVec_Wakeng const& Src,CardStyleNumberVec& style,bool last)
{
	CardVec_Wakeng Normal,Laizi;
	...
	case 3:
		...
		if (IsSingleList(Normal, Laizi, node.m_bCardNumber))	//单顺
		{
...
bool CardStyle_Shanxi_Wakeng::CheckCardStyle(CardVec_Wakeng const& Src,BYTE style,BYTE& cardNumber)
{
	CardVec_Wakeng Normal,Laizi;
	GetLaizi(Src,Laizi,Normal);
	...
	case card_style_single_list:
		if(len >= 3 && len <= 10 && IsSingleList(Normal,Laizi,cardNumber))
		{
			return true;
		}
		return false;
...
////OK;
//bool CardStyle_Shanxi_Wakeng::IsRocket(CardVec_Wakeng const& vec,BYTE& cardNumber)	//是否是火箭;
//{
...
//OK;
bool CardStyle_Shanxi_Wakeng::IsDouble(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)	//是否是对子;
{
	if(laizi.size() == 0)
	{
...
////OK
//bool CardStyle_Shanxi_Wakeng::IsBomb1(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)	//纯软炸弹;
//{
//	if(laizi.size() == 4)
//	{
//		cardNumber = laizi[0]->GetNumberType();
...
//OK
bool CardStyle_Shanxi_Wakeng::IsBomb2(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)	//纯硬炸弹;
{
...
////OK
//bool CardStyle_Shanxi_Wakeng::IsBomb3(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)	//软炸弹;
//{
//	if(laizi.size() == 1)
//	{
...
////OK
//bool CardStyle_Shanxi_Wakeng::IsBombToSingle(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)//四带2单
//{
//	std::size_t sLaizi = laizi.size();
//	CardVec_Wakeng single,duizi,three,four,numnber2,joke,other;
...
////OK
//bool CardStyle_Shanxi_Wakeng::IsBombToDouble(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardType)	//四带2对
//{
//	std::size_t sLaizi = laizi.size();
//	CardVec_Wakeng joke,number2,single,duizi,three,four,other;
...
//OK
bool CardStyle_Shanxi_Wakeng::IsThree(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)	//是否是三张;
{
	if (vec.size() && (vec[0]->GetNumberType() == card_number_type_SJ,
		vec[0]->GetNumberType() == card_number_type_BJ,
		vec[0]->GetNumberType() == card_number_type_LZ))
	{
		return false;
	}
	...
	if(laizi.size() < 3)
	{
		...
		CardVec_Wakeng::const_iterator it = vec.begin();
		for(;it != vec.end();++it)
		{
...
//bool CardStyle_Shanxi_Wakeng::IsThreeList(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardType)	//飞机;
//{
...
////这个函数写的太长了，得想办法优化;
//bool CardStyle_Shanxi_Wakeng::IsThreeListToSingle(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardType)//飞机带单
//{
...
////这个函数最麻烦;
//bool CardStyle_Shanxi_Wakeng::IsThreeListToDouble(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardType)//飞机带对;
//{
//	bool ret = false;
...
////OK
//bool CardStyle_Shanxi_Wakeng::IsThreeToSingle(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardType)//三带1
//{
//	std::size_t sLaizi = laizi.size();
//	CardVec_Wakeng single,duizi,three,four;
...
////OK;
//bool CardStyle_Shanxi_Wakeng::IsThreeToDouble(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardType)//三带对;
//{
//	std::size_t sLaizi = laizi.size();
//	CardVec_Wakeng single,duizi,three,four;
//	GetSingleDoubleThreeFour(vec,single,duizi,three,four);
...
//OK
bool CardStyle_Shanxi_Wakeng::IsSingleList(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)//是否是单顺
{
	BYTE type = vec[0]->GetNumberType();
	Lint lastIndex = (Lint)vec.size() - 1;
	assert(lastIndex > 0);
	BYTE typeLast = vec[lastIndex]->GetNumberType();
	if(type > card_number_type_K)
	{
		return false;
	}
...
//OK
bool CardStyle_Shanxi_Wakeng::IsDoubleList(CardVec_Wakeng const& vec,CardVec_Wakeng const& laizi,BYTE& cardNumber)//是否是双顺
{
	BYTE type = vec[0]->GetNumberType();
	if(type > card_number_type_K )
	{
...
void CardStyle_Shanxi_Wakeng::GetSingleDoubleThreeFour(CardVec_Wakeng const& vec,CardVec_Wakeng& single,CardVec_Wakeng& duizi,CardVec_Wakeng& three,CardVec_Wakeng& four)
{
	single.clear();
	duizi.clear();
	three.clear();
...
void CardStyle_Shanxi_Wakeng::GetJokeNumber2(CardVec_Wakeng const& vec,CardVec_Wakeng& joke,CardVec_Wakeng& number2,CardVec_Wakeng& other)
{
	joke.clear();
	number2.clear();
	other.clear();
...
void CardStyle_Shanxi_Wakeng::GetLaizi(CardVec_Wakeng const& vec,CardVec_Wakeng& laizi,CardVec_Wakeng& other)
{
	laizi.clear();
	other.clear();
...
void CardStyle_Shanxi_Wakeng::GetBombLaiziJokeNumber2(CardVec_Wakeng const& vec,CardVec_Wakeng& bomb,CardVec_Wakeng& laizi,CardVec_Wakeng& joke,CardVec_Wakeng& number2,CardVec_Wakeng& other)
{
	bomb.clear();
	laizi.clear();
	joke.clear();
...
bool CardStyle_Shanxi_Wakeng::CheckContinuous(std::vector<BYTE>& vec)
{
	if(vec.empty())
	{
		return false;
...
//bool CardStyle_Shanxi_Wakeng::HasRocket(CardVec_Wakeng const& vec)
//{
//	Lint nHasBKing = 0;
//	Lint nHasSKing = 0;
...
bool CardStyle_Shanxi_Wakeng::IsBiZhua(CardVec_Wakeng const& vec)
{
	CardVec_Wakeng single,duizi,three,four;
	GetSingleDoubleThreeFour(vec,single,duizi,three,four);
...
bool CardStyle_Shanxi_Wakeng::IsBiDao(CardVec_Wakeng const& vec)
{
	CardVec_Wakeng single,duizi,three,four;
	GetSingleDoubleThreeFour(vec,single,duizi,three,four);
...
//这里主要是给一个全面的评分;
int CardStyle_Shanxi_Wakeng::GetCardScore(CardVec_Wakeng& vec)
{
	int score = 0;
...
CardStyle_Shanxi_Wakeng* CardStyle_Shanxi_Wakeng::Instance()
{
	static CardStyle_Shanxi_Wakeng inc;
	return &inc;
}

//////////////////////////////////////////////////////////////////////////
CardHelp_Shanxi_Wakeng::CardHelp_Shanxi_Wakeng()
{
...
std::vector<std::vector<Lint>> CardHelp_Shanxi_Wakeng::HelpMe(Lint cardMode, Lint cardType, Lint cardCount, CardVec_Wakeng& handcard, std::vector<Lint>& m_curHelpUnusable, bool baojing, bool isUseBomb)
{
	std::vector<std::vector<Lint>> vec;
	CardVec_Wakeng one,tow,three,four;
	CardStyle_Shanxi_Wakeng::Instance()->GetSingleDoubleThreeFour(handcard,one,tow,three,four);
	...
	Lint vecFlag[CARD_COUNT_WAKENG+20] = {0};   // 不在玩家手中牌为0，在手中可出为1，在手中不可出为2;
	...
	for (Lint ii = 0; ii < CARD_COUNT_WAKENG; ++ii)
	{
		if (2 == vecFlag[ii])
		{
...
void CardHelp_Shanxi_Wakeng::CheckOne(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f, std::vector<std::vector<Lint>>& ret,bool baojing)
{
	std::vector<Card_Wakeng*> vec;
...
void CardHelp_Shanxi_Wakeng::CheckTwo(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f, std::vector<std::vector<Lint>>& ret)
{
	std::vector<Card_Wakeng*> vec;
...
//void CardHelp_Shanxi_Wakeng::CheckThreeToOne(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f, std::vector<std::vector<Lint>>& ret)
//{
//	CardVec_Wakeng tmp = o;
...
//void CardHelp_Shanxi_Wakeng::CheckThreeToTwo(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f, std::vector<std::vector<Lint>>& ret)
//{
//	CardVec_Wakeng tmp = o;
//	tmp.insert(tmp.end(),t.begin(),t.end());
//	tmp.insert(tmp.end(),th.begin(),th.end());
...
//void CardHelp_Shanxi_Wakeng::CheckThreeFlyToTwo(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f,CardVec_Wakeng& handcard,Lint count, std::vector<std::vector<Lint>>& ret)
//{
...
//	{
//		CardVec_Wakeng vec;
//		CardVec_Wakeng check(tmp.begin()+i,tmp.end());
...
//			}
//
//			CardVec_Wakeng h = handcard;
//
//			// 提取除了飞机之外剩下的对子,如果够与飞机搭配则认为可以组成合适牌型;
//			CardStyle_Shanxi_Wakeng::Instance()->EarseCard(h,vec);
//            CardVec_Wakeng one,two,three,four;
//			CardStyle_Shanxi_Wakeng::Instance()->GetSingleDoubleThreeFour(h,one,two,three,four);
//			CardVec_Wakeng two_new;
//			//优先级低的先放入提示数组;
...
//void CardHelp_Shanxi_Wakeng::CheckThreeFlyToSingle(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f,CardVec_Wakeng& handcard,Lint count, std::vector<std::vector<Lint>>& ret)
//{
//	if(handcard.size() < (size_t)count)
//		return;
//
...
void CardHelp_Shanxi_Wakeng::CheckOneList(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f,Lint count, std::vector<std::vector<Lint>>& ret)
{
	//首先提取单张;
	CardVec_Wakeng tmp = o;
	for(Lsize i = 0 ; i < t.size(); i += 2)
	...
	//把3, 2和大小王A去掉排除掉;
	while(tmp.size() > 0 && (tmp.front()->GetNumberType() == card_number_type_A || tmp.front()->GetNumberType() == card_number_type_2 || tmp.front()->GetNumberType() == Shanxi_Wakeng::card_number_type_3 || tmp.front()->GetNumberType() == card_number_type_SJ ||tmp.front()->GetNumberType() == card_number_type_BJ))
	{
		tmp.erase(tmp.begin());
	}
...
void CardHelp_Shanxi_Wakeng::CheckTwoList(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f,Lint count, std::vector<std::vector<Lint>>& ret)
{
	CardVec_Wakeng tmp;
	for(Lsize i = 0 ; i+1 < t.size(); i += 2)
	{
		if (t[i]->GetNumberType() <= card_number_type_K)
		{
			tmp.push_back(t[i]);
			tmp.push_back(t[i+1]);
		}
	...
	{
		if (th[i]->GetNumberType() <= card_number_type_K)
		{
			tmp.push_back(th[i]);
			tmp.push_back(th[i+1]);
		}
	...
	{
		if (f[i]->GetNumberType() <= card_number_type_K)
		{
			tmp.push_back(f[i]);
			tmp.push_back(f[i+1]);
		}
...
//void CardHelp_Shanxi_Wakeng::CheckFourToTwo(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f, std::vector<std::vector<Lint>>& ret)
//{
//	CardVec_Wakeng tmp = o;
//	tmp.insert(tmp.end(),t.begin(),t.end());
//	tmp.insert(tmp.end(),th.begin(),th.end());
...
//			CardStyle_Shanxi_Wakeng::Instance()->EarseCard(m,mm);
//
...
void CardHelp_Shanxi_Wakeng::CheckThree(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f, std::vector<std::vector<Lint>>& ret)
{
	for(Lsize i = 0 ; i < th.size(); i+=3)
	{
		if(th[i]->GetNumberType() > cardType)
...
//void CardHelp_Shanxi_Wakeng::CheckThreeList(Lint cardType,CardVec_Wakeng& o,CardVec_Wakeng& t,CardVec_Wakeng& th, CardVec_Wakeng& f,CardVec_Wakeng& handcard,Lint count, std::vector<std::vector<Lint>>& ret)
//{
//	CardVec_Wakeng tmp;
//	for(Lsize i = 0 ; i+2 < th.size(); i += 3)
//	{
