...
//
MiniDumper::MiniDumper(bool headless):m_pExceptionInfo(NULL), m_lTime(0)
{
...
LONG MiniDumper::WriteMiniDump(_EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	HMODULE hDll = NULL;
	TCHAR szDbgHelpPath[2*_MAX_PATH];
	...
	if (GetModuleFileName(NULL, m_szAppPath, _MAX_PATH))
	{
		...
		_tcsncpy_s(szDbgHelpPath, _MAX_PATH, m_szAppPath,_MAX_PATH);
        _tcsncat(szDbgHelpPath,_T("DBGHELP.DLL") ,_MAX_PATH );
