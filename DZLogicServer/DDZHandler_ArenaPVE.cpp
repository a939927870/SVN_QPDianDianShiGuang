...
bool GameHandler_ARENAPVE::startup(Desk *desk)
{
	if (desk)
	{
		clearHandler();
...
void GameHandler_ARENAPVE::clearHandler(void)
{
	_clearGame();
...
void GameHandler_ARENAPVE::_clearGame()
{
	m_nDifen = 2; // 低分首叫地主即变为2;
	m_curPos = 0;
	m_playPos = -1;
	m_ticktime = gWork.GetCurTime().MSecs();
	for (Lint i = 0; i < USER_COUNT_ARENAPVE; ++i)
	{
		m_bomb[i] = 0;
		m_handCard[i].clear();
...
void GameHandler_ARENAPVE::SetDeskDeal()
{
	if (m_pDesk && m_pDesk->m_vip)
		m_pDesk->m_vip->SendInfo();

	dealCard_new();
	Lint userId[USER_COUNT_ARENAPVE] = {0};
	Ldouble score[USER_COUNT_ARENAPVE]={0};
	std::vector<Lint> vec[USER_COUNT_ARENAPVE];
...
void GameHandler_ARENAPVE::QiangDiZhu(Lint nPos)
{
	LMsgS2CAskDiZhu msg;
	msg.m_pos = nPos;
	msg.m_difen = 0;
...
void GameHandler_ARENAPVE::HanderUserQiangDiZhuOper(User* pUser,LMsgC2SQdzOper_Sichuan* msg)
{
	...
	if (m_curPos == msg->m_pos)
	{
		if (OPER_MINGZHUA == msg->m_operType && OPER_YES == msg->m_playerOper) //抓;
		{
			...
			{  //能到这里说明只有最后一家叫了地主;
				...
				CardStyle_ArenaPVE::Instance()->SortCard(m_handCard[m_nDiZhuPos]);
...
void GameHandler_ARENAPVE::HanderUserPlayCard(User* pUser,LMsgC2SUserPlay* msg)
{
	...
	case play_card:
		{
			...
			CardStyle_ArenaPVE::Instance()->CheckCardStyle(tmp,style,tmp.size() == m_handCard[pos].size(), objCardMode);
			int index = CardStyle_ArenaPVE::Instance()->CanBig(m_curPlayCount,m_curCardMode,m_curCardType,style);
			if(index != -1)
			{
...
Lint GameHandler_ARENAPVE::getDeskState()
{
	return m_pDesk->m_deskState;
}

void GameHandler_ARENAPVE::NotPlayCard(Lint pos)
{
	//录像;
	std::vector<Lint> vi;
...
void GameHandler_ARENAPVE::PlayCard(Lint pos, CardVec& card, Lchar card_mode, Lchar card_type)
{
	//录像;
	std::vector<Lint> vec;
	...
	CardStyle_ArenaPVE::Instance()->EarseCard(m_handCard[pos],card);
...
void GameHandler_ARENAPVE::CheckPlayState(Lint pos)
{
	if(CheckPlayEnd(pos))
	{
		return;
...
bool GameHandler_ARENAPVE::CheckPlayEnd(Lint pos)
{
...
void GameHandler_ARENAPVE::SetDeskEnd(Lchar pos)
{
	Lint win[USER_COUNT_ARENAPVE] = {0};
	if (pos < m_pDesk->m_nUserCounts && m_pDesk->m_nUserCounts >= 0)
	{
		win[pos] = 1;
	}

	Ldouble gold[USER_COUNT_ARENAPVE] = {0};
	Lint chuntian[USER_COUNT_ARENAPVE] = {0};
...
void GameHandler_ARENAPVE::GetLaiZi()
{
	srand((int)time(0));
...
void GameHandler_ARENAPVE::SetDeskPlay()
{
	GetLaiZi();
...
bool GameHandler_ARENAPVE::OnUserReconnect(User* pUser)
{
...
void GameHandler_ARENAPVE::SendPlayInfo(bool islast)
{
	for(Lint i = 0 ; i < m_pDesk->m_nUserCounts; ++i)
	{
		LMsgS2COutCard msg;
...
void GameHandler_ARENAPVE::SendPlayCard(Lchar pos, Lchar ret, Lchar action, CardVec& vec, bool isFirstOut)
{
	std::vector<Lint>vv;
...
void GameHandler_ARENAPVE::dealCard_new()
{
	_clearGame();
	for(Lint i = 0; i < m_pDesk->m_nUserCounts; ++i)
	{
	...
	{
		CardStyle_ArenaPVE::Instance()->SortCard(m_handCard[i]);
		if(m_pDesk->m_user[i])
		{
...
void GameHandler_ARENAPVE::ProcessRobot(Lint pos, User * pUser)
{
...
void GameHandler_ARENAPVE::QueryUserOper(Lint nCurPos, Lint nMsgID, Lint nOperType, Lint nPara1)
{
...
void GameHandler_ARENAPVE::BoadCastUserOper(Lint nCurPos, Lint nMsgID, Lint nOperType, Lint nOperRel, Lint isRestart)
{
