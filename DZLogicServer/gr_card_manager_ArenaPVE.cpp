...
//////////////////////////////////////////////////////////////////////////
CardDeal_ArenaPVE::CardDeal_ArenaPVE()
{
...
void CardDeal_ArenaPVE::SS(Lint nCardPos, Lint nColor, Lint nNumber)
{
...
void CardDeal_ArenaPVE::TiaoPai(CardValue tiaopai[], Lint nLenth)
{
...
void CardDeal_ArenaPVE::DealCard(CardVec * receivers, Lint receiversLength, CardValue* changeCard, Lint nChangeLength)
{
#if USEASSERT
...
void CardDeal_ArenaPVE::GetLeftCard(CardVec& leftCard)
{
	leftCard.insert(leftCard.end(), m_pCard.begin() + 51, m_pCard.end());
}

void CardDeal_ArenaPVE::SetLaizi(BYTE bCardNumber)
{
...
void CardDeal_ArenaPVE::ClearLaizi()
{
	for(int i = 0 ; i < m_nSize; ++i)
	{
...
void CardDeal_ArenaPVE::RandCard_15cards()
{
	m_pCard.clear();
...
void CardDeal_ArenaPVE::RandCard()
{
	m_pCard.clear();
	for(int i = 0 ; i < m_nSize; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
CardStyle_ArenaPVE::CardStyle_ArenaPVE()
{
}
...
void CardStyle_ArenaPVE::SortCard(CardVec& vec)
{
	std::sort(vec.begin(),vec.end(),sort_fun);
}

void CardStyle_ArenaPVE::SortCardLaiziFront(CardVec& vec)
{
	std::sort(vec.begin(),vec.end(),sort_fun);
...
void CardStyle_ArenaPVE::SortCardLaiziInsert(CardVec& vec)
{
	std::sort(vec.begin(),vec.end(),sort_fun_by_laizi);
}

void CardStyle_ArenaPVE::EarseCard(CardVec& des,CardVec& src)
{
...
int CardStyle_ArenaPVE::CanBig(DWORD dsCount,BYTE bsStyle ,BYTE bsNumber,CardStyleNumberVec& style)
{
...
bool CardStyle_ArenaPVE::CheckHasBomb(CardVec const& Src, Lint* pBombCard)
{
	if (Src.size() < 4)
	{
		return false;
...
bool CardStyle_ArenaPVE::CheckHasBomb_laizi(CardVec const& Src,CardVec const& laizi, Lint* pBombCard)
{
...
void CardStyle_ArenaPVE::CheckCardStyle(CardVec const& Src, CardStyleNumberVec& style,bool last, Lint nObjCardStyle)
{
	CardVec Normal,Laizi;
...
bool CardStyle_ArenaPVE::CheckCardStyle(CardVec const& Src,BYTE style,BYTE& cardNumber)
{
	CardVec Normal,Laizi;
	GetLaizi(Src,Laizi,Normal);
...
//OK;
bool CardStyle_ArenaPVE::IsRocket(CardVec const& vec,CardVec const& laizi, BYTE& cardNumber)	//是否是火箭;
{
...
//OK;
bool CardStyle_ArenaPVE::IsSuperRocket(CardVec const& vec, CardVec const& laizi, BYTE& cardNumber)	//是否是火箭;
{
#if USEASSERT
	assert(vec.size() >= 2);
#endif
...
//OK;
bool CardStyle_ArenaPVE::IsDouble(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)	//是否是对子;
{
	if(laizi.size() == 0)
	{
...
//OK
bool CardStyle_ArenaPVE::IsBomb1(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)	//纯软炸弹;
{
	if(laizi.size() == 4)
	{
		cardNumber = laizi[0]->GetNumberType();
...
//OK
bool CardStyle_ArenaPVE::IsBomb2(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)	//纯硬炸弹;
{
...
//OK
bool CardStyle_ArenaPVE::IsBomb3(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)	//软炸弹;
{
...
bool CardStyle_ArenaPVE::IsSuperBomb(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)	//软炸弹;
{
	if(laizi.size() == 1)
	{
		if(vec.size() == 4 && vec[0]->GetNumberType() == vec[3]->GetNumberType())
...
bool CardStyle_ArenaPVE::IsDoubleBomb(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)
{
#if USEASSERT
	assert(vec.size() > 0);
#endif
...
//OK
bool CardStyle_ArenaPVE::IsBombToSingle(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)//四带2单
{
...
//OK
bool CardStyle_ArenaPVE::IsBombToDouble(CardVec const& vec,CardVec const& laizi,BYTE& cardType)	//四带2对
{
	std::size_t sLaizi = laizi.size();
...
//OK
bool CardStyle_ArenaPVE::IsThree(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)	//是否是三张;
{
...
bool CardStyle_ArenaPVE::IsThreeList(CardVec const& vec,CardVec const& laizi,BYTE& cardType)	//飞机;
{
...
//这个函数写的太长了，得想办法优化;
bool CardStyle_ArenaPVE::IsThreeListToSingle(CardVec const& vec,CardVec const& laizi,BYTE& cardType)//飞机带单
{
...
//这个函数最麻烦;
bool CardStyle_ArenaPVE::IsThreeListToDouble(CardVec const& vec,CardVec const& laizi,BYTE& cardType)//飞机带对;
{
	bool ret = false;
...
//OK
bool CardStyle_ArenaPVE::IsThreeToSingle(CardVec const& vec,CardVec const& laizi,BYTE& cardType)//三带1
{
	std::size_t sLaizi = laizi.size();
...
//OK;
bool CardStyle_ArenaPVE::IsThreeToDouble(CardVec const& vec,CardVec const& laizi,BYTE& cardType)//三带对;
{
	std::size_t sLaizi = laizi.size();
	CardVec single,duizi,three,four;
	GetSingleDoubleThreeFour(vec,single,duizi,three,four);
...
//OK
bool CardStyle_ArenaPVE::IsSingleList(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)//是否是单顺;
{
	BYTE type = vec[0]->GetNumberType();
	if(type > card_number_type_A)
	{
...
//OK
bool CardStyle_ArenaPVE::IsDoubleList(CardVec const& vec,CardVec const& laizi,BYTE& cardNumber)//是否是双顺;
{
	BYTE type = vec[0]->GetNumberType();
	if(type > card_number_type_A )
	{
...
void CardStyle_ArenaPVE::GetSingleDoubleThreeFour(CardVec const& vec,CardVec& single,CardVec& duizi,CardVec& three,CardVec& four)
{
	single.clear();
	duizi.clear();
	three.clear();
...
void CardStyle_ArenaPVE::GetJokeNumber2(CardVec const& vec,CardVec& joke,CardVec& number2,CardVec& other)
{
	joke.clear();
	number2.clear();
	other.clear();
...
void CardStyle_ArenaPVE::GetLaizi(CardVec const& vec,CardVec& laizi,CardVec& other)
{
	laizi.clear();
	other.clear();
...
void CardStyle_ArenaPVE::GetBombLaiziJokeNumber2(CardVec const& vec,CardVec& bomb,CardVec& laizi,CardVec& joke,CardVec& number2,CardVec& other)
{
	bomb.clear();
	laizi.clear();
	joke.clear();
...
bool CardStyle_ArenaPVE::CheckContinuous(std::vector<BYTE>& vec)
{
	if(vec.empty())
	{
		return false;
...
bool CardStyle_ArenaPVE::HasRocket(CardVec const& vec)
{
	Lint nHasBKing = 0;
	Lint nHasSKing = 0;
...
bool CardStyle_ArenaPVE::Has2Count4Bomb(CardVec const& vec)
{
	Lint nHas2 = 0;
	for (size_t i = 0; i < vec.size(); ++i)
	{
...
//这里主要是给一个全面的评分;
int CardStyle_ArenaPVE::GetCardScore(CardVec& vec)
{
	int score = 0;
...
CardStyle_ArenaPVE* CardStyle_ArenaPVE::Instance()
{
	static CardStyle_ArenaPVE inc;
	return &inc;
}

//////////////////////////////////////////////////////////////////////////
CardHelp_ArenaPVE::CardHelp_ArenaPVE()
{
...
//std::vector<std::vector<Lint>> CardHelp_ArenaPVE::HelpMe_ArenaPVE(Lint cardMode, Lint cardType, Lint cardCount, CardVec& handcard, std::vector<Lint>& m_curHelpUnusable, bool baojing)
//{
//	std::vector<std::vector<Lint>> vec;
//	return vec;
//}

std::vector<std::vector<Lint>> CardHelp_ArenaPVE::HelpMe(Lint cardMode, Lint cardType, Lint cardCount, CardVec& handcard, std::vector<Lint>& m_curHelpUnusable, bool baojing)
{
	//只对普通牌进行判牌;
	std::vector<std::vector<Lint>> vec;
	CardVec normal, laizi;
	CardStyle_ArenaPVE::Instance()->GetLaizi(handcard, laizi, normal);

	//
	CardVec one,tow,three,four;
	CardStyle_ArenaPVE::Instance()->GetSingleDoubleThreeFour(normal,one,tow,three,four);
...
void CardHelp_ArenaPVE::CheckOne(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f, std::vector<std::vector<Lint>>& ret,bool baojing)
{
	std::vector<Card*> vec;
...
void CardHelp_ArenaPVE::CheckTwo(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f, std::vector<std::vector<Lint>>& ret)
{
	std::vector<Card*> vec;
...
void CardHelp_ArenaPVE::CheckThreeToOne(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f, std::vector<std::vector<Lint>>& ret)
{
	CardVec tmp = o;
	tmp.insert(tmp.end(),t.begin(),t.end());
	tmp.insert(tmp.end(),th.begin(),th.end());
	tmp.insert(tmp.end(),f.begin(),f.end());
	CardStyle_ArenaPVE::Instance()->SortCard(tmp);
	if(tmp.size() < 4)
		return;
    CardVec th_new;
...
void CardHelp_ArenaPVE::CheckThreeToTwo(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f, std::vector<std::vector<Lint>>& ret)
{
	CardVec tmp = o;
	tmp.insert(tmp.end(),t.begin(),t.end());
	tmp.insert(tmp.end(),th.begin(),th.end());
...
void CardHelp_ArenaPVE::CheckThreeFlyToTwo(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f,CardVec& handcard,Lint count, std::vector<std::vector<Lint>>& ret)
{
	...
	for(Lsize i = 0 ; i+step*3 <= tmp.size(); i += 3)
	{
		...
		if(vec.size() == step*3 && vec.front()->GetNumberType() > cardType)
		{
			...
			CardStyle_ArenaPVE::Instance()->EarseCard(h,vec);
            CardVec one,two,three,four;
			CardStyle_ArenaPVE::Instance()->GetSingleDoubleThreeFour(h,one,two,three,four);
			CardVec two_new;
			//优先级低的先放入提示数组;
...
void CardHelp_ArenaPVE::CheckThreeFlyToSingle(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f,CardVec& handcard,Lint count, std::vector<std::vector<Lint>>& ret)
{
	if(handcard.size() < (size_t)count)
		return;
...
void CardHelp_ArenaPVE::CheckOneList(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f,Lint count, std::vector<std::vector<Lint>>& ret)
{
	//首先提取单张;
	CardVec tmp = o;
	for(Lsize i = 0 ; i < t.size(); i += 2)
...
void CardHelp_ArenaPVE::CheckTwoList(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f,Lint count, std::vector<std::vector<Lint>>& ret)
{
	CardVec tmp;
...
void CardHelp_ArenaPVE::CheckFourToTwo(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f, std::vector<std::vector<Lint>>& ret)
{
	CardVec tmp = o;
	tmp.insert(tmp.end(),t.begin(),t.end());
	tmp.insert(tmp.end(),th.begin(),th.end());
	...
	for(Lsize i = 0 ; i+3 < f.size(); i+=4)
	{
		if(f[i]->GetNumberType() > cardType)
		{
			...
			CardStyle_ArenaPVE::Instance()->EarseCard(m,mm);
...
void CardHelp_ArenaPVE::CheckThree(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f, std::vector<std::vector<Lint>>& ret)
{
	for(Lsize i = 0 ; i+2 < th.size(); i+=3)
	{
		if(th[i]->GetNumberType() > cardType)
...
void CardHelp_ArenaPVE::CheckThreeList(Lint cardType,CardVec& o,CardVec& t,CardVec& th, CardVec& f,CardVec& handcard,Lint count, std::vector<std::vector<Lint>>& ret)
{
	CardVec tmp;
	for(Lsize i = 0 ; i+2 < th.size(); i += 3)
	{
