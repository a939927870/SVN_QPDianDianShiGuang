...
bool HttpRequest::HttpGet(std::string httpurl, std::string& result, bool isHttps)
{
	...
	else
	{
		...
		result = recvBuff;
		std::size_t htmlfind = result.find("<html>");
		if (htmlfind != std::string::npos)
		{
			LLOG_ERROR("HttpRequest::HttpGet Get Html Result:%s ", result.c_str());
			result.clear();
			return false;
		}
		return true;
...
bool HttpRequest::HttpPost(std::string httpurl,std::string postData, std::string& result,bool isHttps)
{
